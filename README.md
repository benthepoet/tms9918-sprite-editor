...
3. Click **+ Frame** or press `Ctrl+Shift+F` to capture all **stacked** sprites into a new frame.
...
In **static edit**, capturing a frame copies only **stacked** project sprites (checked stack boxes). Capture is blocked with an error if nothing is stacked.

In **frame edit**, the sprite list shows that frame's slots and all sprites are always stacked (stack checkboxes are disabled). Adding or removing a sprite updates every other frame in the animation to keep slot counts aligned. Padded slots on other frames are empty and stacked by default. Commit saves the current frame; discard reverts it and removes auto-padded slots from other frames.
